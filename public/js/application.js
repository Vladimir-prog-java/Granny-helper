...
if (registerForm) {
  registerForm.addEventListener("submit", async (event) => {
    ...
    const { password, email, username, action, method, confirmPassword } =
      event.target;
    ...
    if (jsonData.registrated) {
      ...
      const answer = prompt(
        "Такой бабушки нет! Хотите зарегистрировать свою бабушку?"
      );
...
if (loginForm) {
  loginForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    const { password, email, action, method } = event.target;
